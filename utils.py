...
def plot_comparism(pb, no, noise_ratio, out_masked, yout, X_train, Y_train, ModelInfos, num_samp, res, myloss, accuracy):
    params = dict()
    ...
    fig = plt.figure(figsize=((5+2)*3, (5+0.5)*2))
    ...
    if pb == 'darcyPWC':
        title = r"$|\lambda(s) - \hat \lambda(s)|$, RelL2Err = %.4f, Acc = %.2f %%"%(myloss.rel_single(X_learned, X_train).item(), accuracy)
    else:
        title = r"$|\lambda(s) - \hat \lambda(s)|$, RelL2Err = %.4f"%(myloss.rel_single(X_learned, X_train).item())
    plt.subplot(2, 3, 3)
    plt.xlabel('x')#, fontsize=16, labelpad=15)
    plt.ylabel('y')#, fontsize=16, labelpad=15)
    plt.title(title)
    plt.imshow(np.abs(X_learned[0] - X_train[0]), cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto')#, vmin=0, vmax=1, )
    plt.colorbar()#format=OOMFormatter(-5))
    ...
    plt.xlabel('x')#, fontsize=16, labelpad=15)
    plt.ylabel('y')#, fontsize=16, labelpad=15)
    plt.title(r"$|u(s) - \hat u(s)|$, RelL2Err = "+str(round(myloss.rel_single(Y_learned, Y_train).item(), 3)))
    plt.imshow(np.abs(Y_learned[0] - Y_train[0]), cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto')#, vmin=0, vmax=1, )
    plt.colorbar()#format=OOMFormatter(-5))

    directory = 'figures/'+pb+'/noiseRatio=%s'%(noise_ratio)+'/'+no
