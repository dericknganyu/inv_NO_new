...
ResultsDir = '/localdata/Derick/stuart_data/Darcy_421/INV_NO/'
...
file.write("Alpha Weight_Decay Accuracy Input_Error Output_Error Output_Error_Noiseless\n")

samp = 0
WD    = [2.5e-5, 2.5e-6,  2.5e-4, 2.5e-3, 2.5e-2, 0, 1e-5, 5e-5,    5e-6,   1e-6,  5e-4,  1e-4, 5e-3, 1e-3, 5e-2, 1e-2] + [1, 5] #[5e-4]# 
ALPHA = [0.025, 0.00025,  0.0025,   0.25,    2.5, 0, 0.01, 0.05 , 0.0001, 0.0005, 0.001, 0.005,  0.1,  0.5,    1,    5] + [5e-5, 1e-5, 5e-6, 1e-6] #[5e-2]# 
GM = []
SS = []
for wd in WD:
    for alpha in ALPHA:
        ...
        directory = ResultsDir +  'files/'+pb+'/noiseRatio=%s'%(noise_ratio)
        if not os.path.exists(directory):
            os.makedirs(directory)
        ...
        # epochs = 2000 if pb == 'darcyPWC' and (no == 'pcalin' or no == 'pcann') else 10000
        for ep  in range(epochs):
            ...
            TV_loss = total_variance if pb == 'darcyPWC' or pb == 'structuralMechanics' else tvl2
            ...
            if pb == 'darcyPWC':
                ModelInfos = "_%s-%s_%s_%03d"%(no, pb, init, res)+"~res_"+"_%05d"%(ep+1)+"~ep_"+"%.4f~InAcc_"%(accuracy)+str(np.round(testx_l2,6))+"~InputError_"+str(np.round(testy_l2_noisy,6))+"~OutputError_"+\
                ...
                ModelInfos = "_%s-%s_%s_%03d"%(no, pb, init, res)+"~res_"+"_%05d"%(ep+1)+"~ep_"+str(np.round(testx_l2,6))+"~InputError_"+str(np.round(testy_l2_noisy,6))+"~OutputError_"+\
                    str(np.round(testy_l2,6))+"~OutputErrorNoiseless_"+str(learning_rate)+"~LR_"+str(wd)+"~Reg_"+str(gamma)+"~gamma_"+str(step_size)+"~Step_"+str(epochs)+"~epochs_"+time.strftime("%Y%m%d-%H%M%S")  
        ...
        #plt.show()
        file = open(directory+'/Results_%s_samples_'%(ntest)+TIMESTAMP+'.txt',"a")
        file.write("%.8f %.8f %.6f %.6f %.6f %.6f\n"%(alpha, wd, accuracy, testx_l2, testy_l2_noisy, testy_l2))
